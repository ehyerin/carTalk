						...
						<c:forEach var="vo" items="${requestScope.lvo.list}">
							<tr>
								...
								<td class="titleView"><a
									href="${initParam.root}trade_showContent.do?tradeNo=${vo.tradeNo }">${vo.tradeTitle}</a>
									&nbsp;[${vo.tradeCommentCount }]
								</td>
								<td>${vo.memberId}</td>
								<td>${vo.tradeTimePosted }</td>
								<td>${vo.tradeHit }</td>
				...
				<div class="col-md-12">
					<ul class="pager">
						<li><c:if
								...
								<a
									href="${initParam.root}customercenter_trade_list.do?pageNo=${requestScope.lvo.pagingBean.
						...
						<li><c:forEach var="i"
								...
								<c:choose>
									<c:when test="${requestScope.lvo.pagingBean.nowPage!=i}">
										<a
											href="${initParam.root}customercenter_trade_list.do?pageNo=${i}">${i}</a>
									</c:when>
									<c:otherwise>
							${i}
						</c:otherwise>
